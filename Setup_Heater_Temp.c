...
void Do_Setup_Heater_Temp(void)
{
    ...
    while (In_Heater_Temp_Setup == 1)
    {
         if (check_for_button_input() == 1)
        {
             ...
             {
                Do_Beep_Good(); 
                // Code to handle 'Play/Pause', 'Minus', 'Plus' and 'EQ'
...
{
    setup_heater_set_temp++;    // Increment set_temp by 1
    if (setup_heater_set_temp > 110) setup_heater_set_temp = 110; // Set upper limit to 110
    Update_Setup_Heater_Screen();                       // Update screen with latest info
}

void Decrease_Heater_Temp()          
{
    --setup_heater_set_temp;    // Decrement set_temp by 1
    if (setup_heater_set_temp < 50) setup_heater_set_temp = 50; // Set lower limit to 50
    	Update_Setup_Heater_Screen();                      // Update screen with latest info    
}
